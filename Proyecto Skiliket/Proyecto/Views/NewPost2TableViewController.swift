...
protocol NewPost2TableViewControllerDelegate: AnyObject {
    func didSelectProject(_ project: Project)
}

class NewPost2TableViewController: UITableViewController {
    
    var projectsList = [Project]()
    var selectedProjectIndex: IndexPath?
    weak var delegate: NewPost2TableViewControllerDelegate?
    ...
    private func setupTableHeaderView() {
        ...
        headerLabel.text = "Select one project"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "NewPost2TableViewCell", for: indexPath) as! NewPost2TableViewCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let selectedIndex = selectedProjectIndex, selectedIndex != indexPath {
            let previousCell = tableView.cellForRow(at: selectedIndex) as? NewPost2TableViewCell
            previousCell?.isChecked = false
        }

        let cell = tableView.cellForRow(at: indexPath) as? NewPost2TableViewCell
        cell?.isChecked = true
