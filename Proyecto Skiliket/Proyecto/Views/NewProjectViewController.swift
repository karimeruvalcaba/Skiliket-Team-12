...
class NewProjectViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITableViewDelegate, UITableViewDataSource {

    @IBOutlet weak var newProjectScrollView: UIScrollView!
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        if let keyboardFrame = notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
            ...
            newProjectScrollView.contentInset = contentInsets
            newProjectScrollView.scrollIndicatorInsets = contentInsets
        }
    }
    ...
    @objc func saveChangesButtonTapped() {
        let alert = UIAlertController(title: "Success", message: "Project created", preferredStyle: .alert)
    ...
    func adjustYPositions() {
        ...
        let scrollViewWidth = newProjectScrollView.frame.width - 2 * padding
    ...
    func lockScrollViewToVertical() {
        newProjectScrollView.contentSize = CGSize(width: newProjectScrollView.frame.width, height: solutionsTable.frame.maxY + 20)
        newProjectScrollView.showsHorizontalScrollIndicator = false
        newProjectScrollView.alwaysBounceHorizontal = false
    }
    ...
    // MARK: - UITableViewDelegate: Handle selection for "+" button
    ...
    // MARK: - UIPickerViewDataSource: Required Methods
